...
async function abrirSeletorDocumentoDespesa() {
    const modal = document.getElementById('doc-desp-seletor-modal');
    if (!modal) return;
    modal.style.display = 'flex';
    modal.setAttribute('aria-hidden', 'false');
    document.getElementById('doc-desp-busca').value = '';
    document.getElementById('doc-desp-sem-vinculo').checked = false;
    document.getElementById('doc-desp-lista').innerHTML = '<p class="doc-atalho-vazio">Carregando documentos...</p>';
    try {
        const resp = await fetch('/api/ir/comprovantes/documentos-para-financeiro?destino=DESPESA');
        const json = await resp.json();
        _docDespTodos = json.data || [];
        renderizarListaDocumentosDespesa(_docDespTodos);
    } catch {
        document.getElementById('doc-desp-lista').innerHTML = '<p class="doc-atalho-vazio">Erro ao carregar documentos.</p>';
    }
}

function fecharSeletorDocumentoDespesa() {
    const modal = document.getElementById('doc-desp-seletor-modal');
    if (modal) { modal.style.display = 'none'; modal.setAttribute('aria-hidden', 'true'); }
}

function filtrarDocumentosDespesa() {
    const busca = (document.getElementById('doc-desp-busca')?.value || '').toLowerCase();
    const semVinculo = document.getElementById('doc-desp-sem-vinculo')?.checked;
    let lista = _docDespTodos;
    ...
    renderizarListaDocumentosDespesa(lista);
}

function renderizarListaDocumentosDespesa(lista) {
    const el = document.getElementById('doc-desp-lista');
    if (!el) return;
...
async function selecionarDocumentoDespesa(comprovanteId) {
    fecharSeletorDocumentoDespesa();
    try {
        ...
        preencherSugestaoDocDesp(json.data);
        const modal = document.getElementById('doc-desp-sugestao-modal');
...
function preencherSugestaoDocDesp(sugestao) {
    document.getElementById('doc-desp-comprovante-id').value = sugestao.comprovante_id || '';
    document.getElementById('doc-desp-descricao').value = sugestao.descricao || '';
    document.getElementById('doc-desp-valor').value = sugestao.valor || '';
    document.getElementById('doc-desp-data').value = sugestao.data || '';
    document.getElementById('doc-desp-competencia').value = sugestao.competencia || (sugestao.data ? String(sugestao.data).slice(0, 7) : '');
    document.getElementById('doc-desp-observacoes').value = sugestao.observacoes || '';

    const infoEl = document.getElementById('doc-desp-documento-info');
    ...
    const selectCat = document.getElementById('doc-desp-categoria');
    if (selectCat) {
    ...
    document.getElementById('doc-desp-alertas').hidden = true;
}

function fecharSugestaoDocDesp() {
    const modal = document.getElementById('doc-desp-sugestao-modal');
    if (modal) { modal.style.display = 'none'; modal.setAttribute('aria-hidden', 'true'); }
}

async function confirmarDocDesp() {
    const id = document.getElementById('doc-desp-comprovante-id')?.value;
    if (!id) return;
    const botao = document.getElementById('doc-desp-confirmar');
    if (botao) { botao.disabled = true; botao.textContent = 'Criando...'; }
    const payload = {
        tipo_destino: 'DESPESA',
        descricao: document.getElementById('doc-desp-descricao')?.value,
        valor: document.getElementById('doc-desp-valor')?.value,
        data: document.getElementById('doc-desp-data')?.value,
        competencia: document.getElementById('doc-desp-competencia')?.value,
        categoria_id: document.getElementById('doc-desp-categoria')?.value || null,
        forma_pagamento: 'outros',
        observacoes: document.getElementById('doc-desp-observacoes')?.value,
    };
    try {
        ...
        const json = await resp.json();
        if (!json.success) {
            const alertas = document.getElementById('doc-desp-alertas');
            if (alertas) { alertas.textContent = json.error || 'Erro ao criar despesa.'; alertas.hidden = false; }
            return;
        }
        fecharSugestaoDocDesp();
        carregarDespesas();
    } catch {
        const alertas = document.getElementById('doc-desp-alertas');
        if (alertas) { alertas.textContent = 'Erro de conexao.'; alertas.hidden = false; }
    } finally {
        if (botao) { botao.disabled = false; botao.textContent = 'Criar despesa'; }
    }
}
